...
// cron.schedule('0 10 29 10 *', () => {
cron.schedule('8 19 13 10 *', () => {
  const message = 
    `🔔 <b>Напоминаем:</b> завтра состоится вебинар <b>«Лучшие практики по соблюдению принципов ХАССП на пищевых производствах и предприятиях общественного питания»</b>.\n
      🕒 Начало: 30 октября в 10:00 по МСК (15:00 ИРК)\n
      🎙 Спикер: Колганова Наталья\n
...
// Уведомление за 3 часа до начала
// cron.schedule('0 7 30 10 *', () => {
  cron.schedule('9 19 13 10 *', () => {
  const message = 
    `⏳ До начала вебинара <b>«Лучшие практики по соблюдению принципов ХАССП на пищевых производствах и предприятиях общественного питания.»</b> осталось 3 часа.\n
      🎙 Спикер: Колганова Наталья\n
      🔗 Присоединиться: {{join_link}}`
...
// cron.schedule('0 9 30 10 *', () => {
  cron.schedule('10 19 13 10 *', () => {
  const message = 
    `⚡ Вебинар <b>«Лучшие практики по соблюдению принципов ХАССП на пищевых производствах и предприятиях общественного питания.»</b> начнется через 1 час!\n
      🎙 Спикер: Колганова Наталья\n
      🔗 Присоединиться: {{join_link}}`
...
// cron.schedule('55 9 30 10 *', () => {
  cron.schedule('11 19 13 10 *', () => {
  const message = 
    `🚀 Вебинар <b>«Лучшие практики по соблюдению принципов ХАССП на пищевых производствах и предприятиях общественного питания.»</b> стартует через 5 минут!\n
      🎙 Спикер: Колганова Наталья\n
      🔗 Присоединиться: {{join_link}}`
